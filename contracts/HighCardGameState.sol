...
contract MentalPoker {
  function pointMultiply(uint256, uint256, uint256) public pure returns (uint256, uint256) {}
  function revealCard(uint256, uint256, uint256, uint256) public view returns (string memory, bool) {}
  function highCard(string memory, string memory) public view returns (string memory, bool) {}
  function cardEqual(string memory, string memory) public pure returns (bool) {}
}
...
contract HighCardGameState {
    ...
    function isValidStateTransition(bytes memory encodedPreviousState, bytes memory encodedNewState, address[2] memory participants, uint256 smallBlind) public view returns (bool) {
        ...
        if (newState.currentBalances[0] + newState.currentBalances[1] + newState.pot !=200*smallBlind) {
            return false;
        }
        ...
        if (newState.handNumber==oldState.handNumber && newState.handRound==oldState.handRound+1) {
            ...
            if (newState.lastActionType == callAction) {
                if (newState.toCall != 0 || oldState.currentBalances[actorIndex] - newState.currentBalances[actorIndex] != oldState.toCall) {
                    return false;
                }
                if (newState.pot != oldState.pot+oldState.toCall) {
                    return false;
                }
            } else if (newState.lastActionType == raiseAction) {
                if (newState.toCall == 0 || oldState.currentBalances[actorIndex] - newState.currentBalances[actorIndex] != newState.toCall + oldState.toCall) {
                    return false;
                }
                if (newState.pot != newState.toCall+oldState.toCall+oldState.pot) {
                    return false;
                }
                ...
                if (oldState.lastActionType == foldAction) {
                    if (actorIndex != winnerIndex || oldState.pot != newState.currentBalances[actorIndex] - oldState.currentBalances[actorIndex]) {
                        return false;
                    } 
                }
                if (oldState.lastActionType == revealAction) {
                    if (oldState.pot != newState.currentBalances[winnerIndex] - oldState.currentBalances[winnerIndex]) {
                        return false;
                    }
        ...
        } else if (newState.handNumber==oldState.handNumber+1 && newState.handRound==1 && oldState.lastActionType==commitAction) {
            if (oldState.currentBalances[(actorIndex+1)%2] - newState.currentBalances[(actorIndex+1)%2] != 2*smallBlind) {
                return false;
            }
            if (newState.lastActionType == callAction) {
                if (newState.pot != 4*smallBlind || newState.toCall != 0) {
                    return false;
                }
                if (oldState.currentBalances[actorIndex] - newState.currentBalances[actorIndex] != 2*smallBlind) {
                    return false;
                }
            } else if (newState.lastActionType == raiseAction) {
                if (newState.pot != oldState.currentBalances[actorIndex] - newState.currentBalances[actorIndex] + 2*smallBlind) {
                    return false;
                }
                if (newState.toCall != oldState.currentBalances[actorIndex] - newState.currentBalances[actorIndex] - 2*smallBlind) {
                    return false;
                }
            } else if (newState.lastActionType == foldAction) {
                if (newState.pot != 3*smallBlind || newState.toCall != 0) {
                    return false;
                }
                if (oldState.currentBalances[actorIndex] - newState.currentBalances[actorIndex] != smallBlind) {
                    return false;
                }
            } else {
                return false;
    ...
    function initialEncodedState(uint256 smallBlind) public view returns (bytes memory) {
        State memory state;
        state.currentBalances[0] = smallBlind*100;
        state.currentBalances[1] = smallBlind*100;
